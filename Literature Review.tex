...
Understanding existing knowledge in Machine Learning, specifically when used to predict financial indicators, helps to contextualise the research completed in this project and position it within the existing field. 
...
The correct predictions can not only allow organisations and individuals to profit greatly but also to avoid loss.
...
There are many financial time series (FTS) prediction methods both theoretical and practical that have attracted attention.
...
Additionally, fundamental analysis frequently makes use of techniques such as sentiment analysis that regularly deal with unstructured data.
...
Traditionally, technical analysis has relied heavily on statistical models to forecast the future performance of assets\cite{rockefeller2019technical} and the act of utilising past values to predict future values has been widely implemented for years.
...
The general premise is that as interest rates grow, the growth of inflation should slow and inflation should eventually decrease. 
...
There are many different types of ANNs.
Two of the ANNs that commonly occur in the literature for predicting financial time series are feedforward neural networks (FNNS) and recurrent neural networks (RNNs).
Feedforward neural networks and recurrent neural networks are differentiated by the direction through the network in which the data flows.

In FNNs the data flow is unidirectional, this means that it flows forward through the network from one layer to the next.
This was the first type of neural network introduced in 1958 by Frank Rosenblatt\cite{Rosenblatt_1957_6098}.
In more recent times, a method called backpropagation\cite{Rumelhart1986-ok} has been introduced to help train feedforward networks.
Backpropagation essentially works by sending the error from an output back through the network and optimising the weights and biases along the way.

The data in RNNs can flow in a loop by using recurrent connections.
This allows RNNs to keep an internal memory of past data that can affect future data and calculations.
The ability to store past data makes RNNs useful for tasks involving sequential data such as speech recognition\cite{sak2014long} or time series analysis\cite{petneházi2019recurrent}.

ANNs are composed of three types of layers: the input, hidden, and output layers.
...
Mean absolute error (MAE), Mean squared error (MSE), Mean absolute percentage error (MAPE), Root mean absolute error (RMAE), Normalised mean square error (NMSE), Root mean squared error (RMSE), Relative root mean squared error (RRMSE), and Correlation coefficient of prediction (R).
...
Instead, the main focus was on understanding the most common techniques used to predict financial time series and which machine learning models are often applied to these tasks.
The literature review also covered the UK's relationship with inflation, who is tasked with controlling it, how they attempt to do so, as well as some of the factors contributing to it.
