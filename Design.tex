...
As each of these metrics are straightforward to implement, they will all be used for evaluating the models created.
...
This is because normalisation will preserve the distribution of the data by rescaling the data where as standardisation makes the asssumption that the date has a Gaussian distribution and may modify the data's distribution if it is not already Gaussian.
...
This saves both time and resources (as the model implemented form the library is likely to have been optimised) while also providing additional support, if necessary, from the community and documentation.
