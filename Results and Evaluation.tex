...
The variety and contents of the research both past and present are seemingly insurmountable.
...
Reviewing literature ultimately became a constant throughout the project allowing for changes and improvements to be made once new information came to light.
If this project were to be completed again it would be a much smoother process purely because of the literature that has already been read both in and out of this report.
...
The choice was made to include data selection and preprocessing in the design section as implementation of the models could not be started without an already suitable dataset.
Data selection was particularly difficult as inflation is commonly collected as an annual indicator and quarterly at best.
...
This meant that it was necessary to undergo the painstaking task of hundreds of samples of data into an admissible format.
...
If there was only one thing that could be improved about the project it would be the size of the dataset.

\subsubsection{Difficulties in the Implementation}
The issues encountered with the sample size naturally followed through into the implementation process, contributing to subpar results from the models compared to if they had been trained on more samples.
...
Using a small testing set could cause performance estimates to be unreliable.
...
This means that as the years progress more and more data will be produced but the rate of getting new data cannot be increased.
...
Using a smaller set of hyperparameters would reduce the wait time but it may also produce worse results if more optimal hyperparameters could have been found but were not included in the search.
