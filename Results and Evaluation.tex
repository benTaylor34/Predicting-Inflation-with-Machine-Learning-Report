...
The model from figure \ref{fig:ann150} produced the best results out of all of the models implemented to predict inflation.
This is likely due to a number of factors.

Removing the first 150 data points removed some large outliers that would have negatively affected the predictions.
This was confirmed by the fact that the other models also showed an increase in performance once the first 150 rows of data were removed.

Another reason for the ANN's success is that it was the most flexible of the models with the largest number of hyperparameters that could be adjusted.
Interestingly, the best set of hyperparameters found by the grid search was somewhat unexpected.
...
Despite this, it does not seem like the model oscillated or diverged based on the loss graph as it did not display any additional inflections.\\
 ...
 a 0\% dropout rate (meaning that its very unlikely the model came close to overtraining), a learning rate of 0.005
