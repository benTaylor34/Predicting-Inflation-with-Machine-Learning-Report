...
\begin{table}[H]
    ...
    Linear Regression              & 1.3733 & 2.3495 & 1.5564        & -4.4516   \\ \hline
    Random Forest Regression       & 0.9299 & 1.1499 & 1.2844        & -0.3059   \\ \hline
    Support Vector Regression      & 0.9968 & 1.2069 & 1.3483        & -0.4384   \\ \hline
...
The closer the MAE is to 0 the better the performance is.\\
...
The closer an RMSE value is to 0 the better the performance is.\\
...
Yet the two best-performing models both produced exceptionally high MAPE values despite the fact that they would be expected to produce the lowest values.
...
The sklearn metric deals with this by instead producing an arbitrarily high output.
...
Rather, the R-squared metric produces a negative value when the model's fit is worse than the fit of the mean deviation of the data.\\


%%%%TODO:

% - decide whether or not to include the results in the implementation section or in the evaluation section
% - include results in implementation then compare results in the evaluation