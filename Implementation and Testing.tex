...
The different machine learning models were linear regression, random forest, support vector regression, long short-term memory network, and a feedforward neural network.
...
Long short-term memory network (LSTM) and the feedforward neural network (FNN) were implemented with the use of TensorFlow and Keras.
The feedforward neural network, somewhat expectedly, produced the best predictions.
...
The feedforward neural network (FNN) was implemented as one type of ANN.
...
This is because the FNN takes just one row of the X and y variables (X is the features being used to predict and y is the CPI value from the same time period of the given features).
