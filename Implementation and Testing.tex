...
This means that using X data from the year 2000 to predict the value of inflation from the same year is redundant as the inflation data will have already been released around the same time as the X data meaning that it is already available.
Instead, the aim of time series forecasting should be to use past data to predict future data.
In creating a dataset for a machine learning model this takes the form of shifting the target forward in time so that the X data from 2000 is being used to predict the y data from 2001.
...
The project dataset was split into 70\% training data and 30\% as this is a common split.
...
This is demonstrated in the Granger causality tests carried out in the Design chapter as several features are shown to have the potential to predict inflation.
Therefore, as the indication as to whether or not any of the features are redundant, all of the features will be kept in the dataset for training the models.
...
This is possibly due to the fact that dropping values in an LSTM may break down temporal dependency relationships.
...
The hyperparameters that were optimised for the ANN model were the number of nodes per layer, the dropout rate, the learning rate, and the size of the batches.
...
The values were selected as they are commonly used or default settings for these hyperparameters.
...
For the ANN model, it was found that the ideal hyperparameters were 16 nodes per layer, a dropout rate of 0, a learning rate of 0.005, and a batch size of 32.\\
...
Identifying and analysing areas of improvement will be carried out in Chapter 5.

\subsection{The Testing Set}
The testing set is the set of previously unseen data that the models will be tested on.
The set was made up of the final 20\% of the original unshuffled data.
...
This is particularly true for machine learning projects as training the algorithm can often be time-consuming.
...
This meant that the errors produced during the implementation of the project had already been identified and solved by other users, once again making the implementation process much smoother.
...
The metrics were calculated using the testing set against the models' predictions of the testing set.
...
\begin{table}[H]
    ...
    Artificial Neural Network      & 0.0225 & 0.0304 & 4617012820526 & 0.9207    \\ \hline
    \end{tabular}
\end{table}

Of the models shown, linear regression performs the worst and the artificial neural network performs the best.
...
Yet all of the models, including the two best-performing models, produced exceptionally high MAPE values despite the fact that they would be expected to produce the lower values.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{annlossboth.png}
    \caption[ANN Model Loss Over Time (epochs).]{ANN Model Loss Over Time (epochs).}
    \label{fig:annlossboth}
\end{figure}
By observing \ref{fig:annlossboth} it can be seen that the loss plateaus after 50 to 60 epochs.
This means that the model could be trained for much fewer epochs while still producing similar results and saving time.
...
\begin{figure}[H]
    ...
    \caption[ANN Model's Predictions of Seen Data.]{ANN Model's Predictions of Seen Data.}
...
Overtraining began occuring around 300 epochs, furthermore training the models for this many epochs became increasingly time consuming whilst using the grid method for hyperparameter optimisation.
...
Although further comparisons and evaluation will be completed, based on the regression metrics taken the artificial neural network produced the best results for predicting inflation.
